...
My wife studies for her medical exams through online lectures in Korean. She found herself constantly pausing the video to take notes, which slowed her progress significantly. We tried existing dictation tools, but they were terrible for Korean medical lectures — especially when the instructor pronounces English medical terms in Korean (e.g., "페소피지올로지" for pathophysiology, "히스타민" for histamine). Off-the-shelf models simply couldn't handle the constant code-switching between Korean and Romanized English terminology. So I built this LectureNotetaker for her. It captures system audio in real time, transcribes it with medical keyword boosting to handle domain-specific vocabulary, and uses an LLM to clean up the messy output into readable notes — all while automatically mapping each segment to the correct lecture slide. Now she can just watch the lecture and let the tool do the note-taking. I hope this tool can also help other students who share the same struggle.
